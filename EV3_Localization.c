...
int roadColours[2] = { BLACKCOLOR, YELLOWCOLOR };
int numRoadColours = 2;

/*
  SOUND SEQUENCES FOR COLOUR DETECTION
*/
const int tone_data[8][50][3] = {
  {
    { 349, 500, 5 },
    { 466, 500, 5 },
    { 523, 500, 5 },
 ...
 *******************************************************************************************************************************/  
 ...
 int tl, tr, br, bl, robot_x, robot_y, direction, foundTarget=0, tries=0;
...
int find_street(int* coloursArray)   
{
 ...
 while(1) {
  ...
  fprintf(stderr, "Turn_count: %d, Color: %d\n", turn_count, color);
  while (color != BLACKCOLOR && color != YELLOWCOLOR) {
   fprintf(stderr, "Turn_count: %d\n", turn_count);
   if (turn_count > 60) {
    fprintf(stderr, "lost\n");
    return -1;
   }
   ...
   if (color == REDCOLOR) {
    ...
    if (last_turn_direction == 0) last_turn_direction = 1;
    color = turn(coloursArray, -90 * last_turn_direction, 0);
    break;
   }
   // turning clockwise
  ...
  while (1) {
   ...
   while(color == BLACKCOLOR || color == YELLOWCOLOR) {
    fprintf(stderr, "Trying to stay on street: %d\n", black_steps);
    if (black_steps >= 5) {
...
{
 int angle, rate, totalAdjustment = 0, adjustmentLimit = 20, motorspeed = -35;
 double drive_speed = 8.0;
 ...
 while(1) {
  ...
  // go in opposite direction if we've hit an intersection
...
//  BT_drive(MOTOR_LEFT, MOTOR_RIGHT, base_speed);
 for (int m=0; m<1000; m++) {
  ...
  fprintf(stderr, "drive along colour %d\n", color);

  if (color == YELLOWCOLOR) {
   fprintf(stderr, "HIT YELLOW\n");
   BT_turn(MOTOR_LEFT, 0, MOTOR_RIGHT, 0);
   BT_all_stop(1);
   return YELLOWCOLOR;
  ...
  if (error > 0) {
   ...
   left_speed = base_speed - abs(pid_out);
   right_speed = base_speed + abs(pid_out);
   // Limit speed
...
int scan_intersection(int*coloursArray, int *tl, int *tr, int *br, int *bl)
{
  ...
  *(bl) = coloursDetected[0];
  *(br) = coloursDetected[2];
  ...
  if (*br == BLACKCOLOR || *bl == BLACKCOLOR)
  {
    ...
    BT_drive(MOTOR_LEFT, MOTOR_RIGHT, -15);
    colour = wait_until_get_colours(coloursArray, roadColours, 2);
    ...
    *(br) = coloursDetected[2];
    ...
    BT_drive(MOTOR_LEFT, MOTOR_RIGHT, 17);
  ...
  colour = wait_until_get_colours(coloursArray, roadColours, 1);
  colour = wait_colour_change(coloursArray, BLACKCOLOR);
  ...
  scan_colours(coloursArray, coloursDetected);
  *(tl) = coloursDetected[0];
  *(tr) = coloursDetected[2];
...
{
  int colour = 0, angle=0, rate, lpow = 0, rpow = 0, hitRoad = 0, leeway;
...
int turn_at_intersection(int* coloursArray, int turn_direction)
{
  ...
  */
  ...
  //BT_drive(MOTOR_LEFT, MOTOR_RIGHT, 30);
  //BT_motor_port_stop(MOTOR_LEFT | MOTOR_RIGHT, 0);
  
  // turning left
  if (turn_direction)
  {
    color = turn(coloursArray, -88, 0);
  }
...
int robot_localization(int *coloursArray, int *robot_x, int *robot_y, int *direction)
{
  ...
  */
  ...
  while (i < 10){
    ...
    if (colour == REDCOLOR)
    {
      // go backwards to the previous intersection
      BT_drive(MOTOR_LEFT, MOTOR_RIGHT, -15);
...
int go_to_target(int* coloursArray, int robot_x, int robot_y, int direction, int target_x, int target_y)
{
  ...
  */
  ...
  if ((target_direction == 3 && direction == 2) || (target_direction == 1 && direction == 0)){
    //right turn needed
    turn_at_intersection(coloursArray, 0);
    direction = (direction + 1) %4;
  }
  else if ((target_direction == 3 && direction == 0) || (target_direction == 1 && direction == 2)){
    //left turn needed
    turn_at_intersection(coloursArray, 1);
    direction = direction - 1;
    if (direction < 0) direction = 3;
  }
  else if (direction != target_direction){
    //180 needed
  ...
  //rotate to target direction
  if ((target_direction == 0 && direction == 3) || (target_direction == 2 && direction == 1)){
    //right turn needed
    turn_at_intersection(coloursArray, 0);
    direction = (direction + 1) %4;
  }
  else if ((target_direction == 0 && direction == 1) || (target_direction == 2 && direction == 3)){
    //left turn needed
    turn_at_intersection(coloursArray, 1);
    direction = direction - 1;
    if (direction < 0) direction = 3;
  ...
  BT_play_tone_sequence(tone_data[7]);
  return(1);  
}
...
int wait_until_get_colours(int* coloursArray, int wantedColours[], int numColours)
{
  ...
  for (int i=0; i<100; i++)
  {
    ...
    {
      if (colour == wantedColours[j]) return colour;
    }
    colour = detect_and_classify_colour(coloursArray);
  }
  return colour;
...
{
  int prevAngle, angle, rate, power, prevColour, distance, i, direction;
  double err, prevErr, diff;

  // rotates gyro all the way to the right
  BT_turn(MOTOR_MIDDLE, 80, MOTOR_GHOST, 0);
  ...
  coloursDetected[1] = 0;
  while (angle < 95 || angle >= 100)
  {
    direction = (angle < 95) ? 1 : -1;
    BT_turn(MOTOR_MIDDLE, -1 * direction, MOTOR_GHOST, 0);
    BT_read_gyro(GYRO_PORT, 0, &angle, &rate);
    BT_turn(MOTOR_MIDDLE, 10 * direction, MOTOR_GHOST, 0);
    usleep(100);
    ...
    if ((angle > 93 && (distance <= 44 && distance >= 40)) || (angle >= 93 && (coloursDetected[1] == YELLOWCOLOR || coloursDetected[1] == BLACKCOLOR || coloursDetected[1] == REDCOLOR)))
    {
      break;
    }
  }
...
double pid_controller_update(PIDController *pid, int error, int measurement) {
 double propotional=0.0, integrator=0.0, differentiator=0.0;
 ...
 for (int i=0; i<pid->arr_size; i++) {
  ...
  fprintf(stderr, "IIIIIIIIIIintegrator: %.2f\n", integrator);
 }
